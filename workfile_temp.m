...
numchanreal=1; % Number of channel realizations
numfiltreal=4; % Number of filter realizations per channel
...
numdatasamples = 2*numants*(spreadgain+delayspread-1);
...
beamsteer = permute(mpath_aoa_aug,[2 3 1]); % numants-by-delayspread-by-numusers
...
% Conditional opt
[coeffs,auxvecs] = condauxvec_coeffs(corrmat,rakevec,numauxvecs);

% Unconditional opt
coeffs_uncond = uncondauxvec_coeffs(corrmat,auxvecs,rakevec);
...
thisber = get_ber_approx(energy(1),coeffs_uncond,rakevec,intnocorrmat);
ber_uncond(snridx) = ber_uncond(snridx) + thisber;
...
figure;
semilogy(user1snrdb,ber_uncond,'o-','markersize',4);
grid on;